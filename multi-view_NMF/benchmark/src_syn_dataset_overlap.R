...
#
    ...
    M = 90; # Number of networks
    dataset = list();
    C_common_1 = 1:60; # Common part 1
    C_rest_1 = setdiff((1:N), C_common_1);

    C_common_2 = 201:240; # Common part 2
    ...
    for(m in 1:M){
        if(m <= 30){
            ...
            increment=m*2
            C_specific = list(C_common_1,s[1:increment]);
        }else if((30 < m) && (m <= 60)){            
            s_1 = C_rest[sample(1:length(C_rest))];
            #s_1 = C_rest;
            C_specific_1 = c(C_common_1, s_1[1:(2 * (61-m))]); # 
            s_1_rest = setdiff(s_1, C_specific_1);

            s_2 = s_1_rest[sample(1:length(s_1_rest))];
            #s_2 = s_1_rest;
            C_specific_2 = c(C_common_2, s_2[1:(2 * (m-30))]) # 
            C_specific = list(C_specific_1, C_specific_2);  
        }else{
            s = C_rest_2[sample(1:length(C_rest_2))];         
            increment=(m-60)*2;
            C_specific = list(C_common_2,s[1:increment]);
        }
        
        p_out = 0.05;
        if( p_out*(N-40) > p_in*(40-1) ){
            p_out = (p_in*(40-1)) / (N-40); # (N-n)*p_out < (n-1)*p_in
        }
        # Background network
