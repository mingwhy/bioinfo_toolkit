%function ming_C3D_r1_coexpr(file_path,qval,vec_n,vad_tag,varargin)
% C3D: main function
%
% Input (mandatory):
...
% ming_C3D_r1_coexpr(file_path, 0.05, 1000, 1, 'abs')
% data_type was forced to 1
% make sure you've installed fdrtool in R and the path to R is correct

file_path= '/Users/mingyang/Downloads/C3DmatR1/brain_10';
qval=0.05;vec_n=900;vad_tag=1;
varargin='abs';
pwd; 
sourcefolder=pwd; 
...
[std,crtag,rnd,rnde,thrvp,ctrig,rab2,abst]=checkInputs(varargin); %Ming debug
...
% Ming, look into V and Lambda
size(V) %10 mat, 0.567130 minutes
size(Lambda)
V(1:3,1:3)
Lambda(1:3,1:3)
...
if ~isempty(key) 
    ...
    % Ming, start: select genes for each column in V
    for k=1:vec_n
        cd(sourcefolder);
        vx=V(:,key(k));
        ...
        %!unset DYLD_LIBRARY_PATH; /usr/bin/Rscript fdrqval.r % Ming
        !unset DYLD_LIBRARY_PATH; /usr/local/bin/Rscript fdrqval.r
    ...
    % Ming, end: select genes for each column in V
    ...
    time_int=toc/60;
    fprintf('Step 3: overlap is Done: %f minutes\n',time_int);
